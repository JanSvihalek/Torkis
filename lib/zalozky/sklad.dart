...
class _StavSkladuTabState extends State<_StavSkladuTab> {
          ...
          child: StreamBuilder<QuerySnapshot>(
            ...
            builder: (context, snapshot) {
              ...
              final docs = snapshot.data!.docs.where((doc) {
                ...
                final searchField = "${data['nazev']} ${data['kod']} ${data['vyrobce']}".toLowerCase();
                return searchField.contains(_searchQuery);
              }).toList();

              if (docs.isEmpty) {
                return const Center(child: Text('Nebyly nalezeny žádné skladové karty.', style: TextStyle(color: Colors.grey)));
              }

              docs.sort((a, b) {
                final nameA = (a.data() as Map<String, dynamic>)['nazev']?.toString().toLowerCase() ?? '';
                final nameB = (b.data() as Map<String, dynamic>)['nazev']?.toString().toLowerCase() ?? '';
                ...
                itemBuilder: (context, index) {
                  ...
                  return Card(
                    ...
                    child: ListTile(
                      ...
                      title: Text(data['nazev'] ?? 'Bez názvu', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                      subtitle: Text('Kód: ${data['kod'] ?? '-'} | Výrobce: ${data['vyrobce'] ?? '-'}'),
                      trailing: Column(
                        ...
                        children: [
                          ...
                          Text('${data['cena_prodej'] ?? 0} Kč', style: const TextStyle(fontSize: 12, color: Colors.grey)),
                        ],
                      ),
                      onTap: () => _showEditPartDialog(context, docId, data, isDark),
                    ),
                  );
                },
              );
  ...
  void _showEditPartDialog(BuildContext context, String docId, Map<String, dynamic> existingData, bool isDark) {
    ...
    final vyrobceCtrl = TextEditingController(text: existingData['vyrobce'] ?? '');
    final nakupCenaCtrl = TextEditingController(text: existingData['cena_nakup']?.toString() ?? '');
    final prodejCenaCtrl = TextEditingController(text: existingData['cena_prodej']?.toString() ?? '');
    final minStavCtrl = TextEditingController(text: existingData['min_stav']?.toString() ?? '0');
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    const Text('Úprava karty dílu', style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
                    const SizedBox(height: 20),

                    _buildModalTextField(nazevCtrl, 'Název dílu *', Icons.build, isDark),
                    const SizedBox(height: 15),
                    Row(
                      children: [
                        Expanded(child: _buildModalTextField(kodCtrl, 'Kód / OEM', Icons.qr_code, isDark)),
                        const SizedBox(width: 15),
                        Expanded(child: _buildModalTextField(vyrobceCtrl, 'Výrobce', Icons.precision_manufacturing, isDark)),
                      ],
                    ),
                    const SizedBox(height: 15),
                    Row(
                      children: [
                        Expanded(child: _buildModalTextField(nakupCenaCtrl, 'Nákupní cena', Icons.attach_money, isDark, isNumber: true)),
                        const SizedBox(width: 15),
                        Expanded(child: _buildModalTextField(prodejCenaCtrl, 'Prodejní cena', Icons.price_check, isDark, isNumber: true)),
                      ],
                    ),
                    const SizedBox(height: 15),
                    Row(
                      children: [
                        Expanded(
                          ...
                          child: DropdownButtonFormField<String>(
                            ...
                            items: ['ks', 'l', 'm', 'bal', 'sada'].map((e) => DropdownMenuItem(value: e, child: Text(e))).toList(),
                            onChanged: (v) => setModalState(() => vybranaJednotka = v!),
                          ),
                        ),
                        const SizedBox(width: 15),
                        Expanded(
                          flex: 2,
                          child: _buildModalTextField(minStavCtrl, 'Min. stav', Icons.warning_amber, isDark, isNumber: true),
                        ),
                      ],
                    ),
                    const Padding(
                      padding: EdgeInsets.only(top: 10, left: 5),
                      child: Text('Pro úpravu aktuálního množství použijte záložku Příjem nebo Prodej.', style: TextStyle(fontSize: 12, color: Colors.grey)),
                    ),
                      ...
                      child: ElevatedButton(
                        onPressed: isSaving ? null : () async {
                          if (nazevCtrl.text.trim().isEmpty) {
                            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Název dílu je povinný!')));
                            return;
                          }

                          setModalState(() => isSaving = true);
                          try {
                            double nakup = double.tryParse(nakupCenaCtrl.text.replaceAll(',', '.')) ?? 0.0;
                            double prodej = double.tryParse(prodejCenaCtrl.text.replaceAll(',', '.')) ?? 0.0;
                            double minStav = double.tryParse(minStavCtrl.text.replaceAll(',', '.')) ?? 0.0;
                            ...
                            await FirebaseFirestore.instance.collection('sklad').doc(docId).update(updateData);

                            if (context.mounted) {
                              Navigator.pop(context);
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(content: Text('Díl byl úspěšně upraven.'), backgroundColor: Colors.green),
                              );
                            }
                          } catch (e) {
                            if (context.mounted) {
                        ...
                        child: isSaving 
                            ...
                            : const Text('ULOŽIT ZMĚNY', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                      ),
                    ),
                  ],
                ),
  ...
  Widget _buildModalTextField(TextEditingController ctrl, String label, IconData icon, bool isDark, {bool isNumber = false, bool readOnly = false}) {
    return TextField(
      controller: ctrl,
...
class _PrijemSkladuTabState extends State<_PrijemSkladuTab> {
            ...
            child: Column(
              ...
              children: [
                StreamBuilder<QuerySnapshot>(
                  stream: FirebaseFirestore.instance.collection('sklad').where('servis_id', isEqualTo: globalServisId).snapshots(),
                  builder: (context, snapshot) {
                ...
                const Text('Údaje o dílu', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.blue)),
                const SizedBox(height: 10),
                _buildInput(controller: _nazevCtrl, label: 'Název dílu *', icon: Icons.build),
                const SizedBox(height: 15),
                Row(
                  children: [
                    Expanded(child: _buildInput(controller: _kodCtrl, label: 'Kód / OEM', icon: Icons.qr_code)),
                    const SizedBox(width: 15),
                    Expanded(child: _buildInput(controller: _vyrobceCtrl, label: 'Výrobce', icon: Icons.precision_manufacturing)),
                  ],
                ),
                const SizedBox(height: 15),
                Row(
                  children: [
                    ...
                    Expanded(
                      flex: 3,
                      child: _buildInput(controller: _minStavCtrl, label: 'Min. stav', icon: Icons.warning_amber, isNumber: true),
                    ),
                  ],
                ),
                ...
                const Text('Naskladnění a ceny', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.green)),
                const SizedBox(height: 10),
                Row(
                  children: [
                    Expanded(child: _buildInput(controller: _nakupkaCtrl, label: 'Nákupní cena', icon: Icons.attach_money, isNumber: true)),
                    const SizedBox(width: 15),
                    Expanded(child: _buildInput(controller: _prodejkaCtrl, label: 'Prodejní cena', icon: Icons.price_check, isNumber: true)),
                  ],
                ),
                const SizedBox(height: 15),
                Row(
                  children: [
                    Expanded(
                      flex: 2,
                      child: _buildInput(controller: _mnozstviCtrl, label: 'Množství k přijetí *', icon: Icons.add_shopping_cart, isNumber: true),
                    ),
                    const SizedBox(width: 15),
                    Expanded(
                      flex: 3,
                      child: _buildInput(controller: _dodavatelCtrl, label: 'Dodavatel / Poznámka', icon: Icons.local_shipping),
                    ),
                  ],
                ),
                ...
                SizedBox(
                  ...
                  child: ElevatedButton.icon(
                    ...
                    icon: _isSaving ? const CircularProgressIndicator(color: Colors.white) : const Icon(Icons.download),
                    label: Text(_isSaving ? 'ZPRACOVÁVÁM...' : 'NASKLADNIT DÍL', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
  ...
  Widget _buildInput({required TextEditingController controller, required String label, required IconData icon, bool isNumber = false}) {
    return TextField(
  ...
  Future<void> _prijmoutNaSklad() async {
    ...
    try {
      double mnozstvi = double.tryParse(_mnozstviCtrl.text.replaceAll(',', '.')) ?? 0.0;
      double nakupka = double.tryParse(_nakupkaCtrl.text.replaceAll(',', '.')) ?? 0.0;
      double prodejka = double.tryParse(_prodejkaCtrl.text.replaceAll(',', '.')) ?? 0.0;
      double minStav = double.tryParse(_minStavCtrl.text.replaceAll(',', '.')) ?? 0.0;

      String partIdToLog = '';
      String finalName = _nazevCtrl.text.trim();

      if (_vybranyDilId == null) {
        DocumentReference ref = await FirebaseFirestore.instance.collection('sklad').add({
        ...
        await FirebaseFirestore.instance.collection('sklad').doc(partIdToLog).update({
          'nazev': finalName,
          'kod': _kodCtrl.text.trim(),
          'vyrobce': _vyrobceCtrl.text.trim(),
          'jednotka': _vybranaJednotka,
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Naskladnění proběhlo úspěšně.'), backgroundColor: Colors.green));
      }
    } catch (e) {
...
class _PultovyProdejTabState extends State<_PultovyProdejTab> {
  ...
  final _slevaCtrl = TextEditingController(text: '0'); // NOVÉ: Sleva
  ...
  double get _celkemKosik {
    double sum = 0;
    for (var p in _kosik) {
      double cena = (p['cena_prodej'] ?? 0).toDouble();
      double sleva = (p['sleva'] ?? 0).toDouble();
      sum += (p['mnozstvi'] * cena) * (1 - (sleva / 100));
    }
    return sum;
  }
  ...
  // Funkce pro přidání ruční položky (mimo existující skladové karty)
  void _pridatVlastniPolozku() {
    final nazevManiCtrl = TextEditingController();
    final cenaManiCtrl = TextEditingController();
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('Ruční přidání položky'),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            TextField(
              controller: nazevManiCtrl, 
              decoration: const InputDecoration(labelText: 'Název položky')
            ),
            const SizedBox(height: 10),
            TextField(
              controller: cenaManiCtrl, 
              decoration: const InputDecoration(labelText: 'Cena za ks s DPH'), 
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              if (nazevManiCtrl.text.isEmpty) return;
              setState(() {
                _kosik.add({
                  'sklad_id': null, // Null znamená, že položka není vedena ve skladu (neodečítá se)
                  'nazev': nazevManiCtrl.text,
                  'kod': 'MANUÁLNÍ',
                  'jednotka': 'ks',
                  'cena_prodej': double.tryParse(cenaManiCtrl.text.replaceAll(',', '.')) ?? 0.0,
                  'mnozstvi': 1.0,
                  'sleva': 0.0,
                });
              });
  ...
  void _pridatDoKosiku() {
    ...
    double mnozstvi = double.tryParse(_mnozstviCtrl.text.replaceAll(',', '.')) ?? 1.0;
    double sleva = double.tryParse(_slevaCtrl.text.replaceAll(',', '.')) ?? 0.0;
    if (mnozstvi <= 0) return;

    double aktualniStav = (_vybranyDilData!['skladem'] ?? 0).toDouble();
    if (mnozstvi > aktualniStav) {
      // Položka není ve skladu zablokována, ale pouze zobrazíme varování
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Pozor: Položka není v dostatečném množství skladem. Sklad půjde do mínusu.'), backgroundColor: Colors.orange)
      );
    }

    setState(() {
      int existujiciIndex = _kosik.indexWhere((p) => p['sklad_id'] == _vybranyDilId);
      if (existujiciIndex >= 0) {
        _kosik[existujiciIndex]['mnozstvi'] += mnozstvi;
        _kosik[existujiciIndex]['sleva'] = sleva; // Aplikuje nejnovější slevu
      } else {
        _kosik.add({
            ...
            return Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 20),
                  const Text('Nákupní košík', style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
                  const SizedBox(height: 10),
                  Expanded(
                    child: _kosik.isEmpty 
                      ? const Center(child: Text('Košík je prázdný.', style: TextStyle(color: Colors.grey)))
                      : ListView.builder(
                          itemCount: _kosik.length,
                          itemBuilder: (context, index) {
                            final p = _kosik[index];
                            final slevaStr = (p['sleva'] != null && p['sleva'] > 0) ? ' (Sleva ${p['sleva']} %)' : '';
                            return ListTile(
                              title: Text(p['nazev'], style: const TextStyle(fontWeight: FontWeight.bold)),
                              subtitle: Text('${p['mnozstvi']} ${p['jednotka']} x ${p['cena_prodej']} Kč$slevaStr'),
                              trailing: IconButton(
                  ...
                  DropdownButtonFormField<String>(
                    decoration: const InputDecoration(labelText: 'Forma úhrady', border: OutlineInputBorder()),
                    value: _formaUhrady,
                    items: ['Hotově', 'Kartou', 'Převodem'].map((v) => DropdownMenuItem(value: v, child: Text(v))).toList(),
                  ...
                  Row(
                    ...
                    children: [
                      const Text('Celkem k úhradě:', style: TextStyle(fontSize: 16)),
                      Text('${_celkemKosik.toStringAsFixed(2)} Kč', style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: Colors.green)),
                    ],
                  ),
                  const SizedBox(height: 20),
                  SizedBox(
  ...
  Future<void> _dokoncitProdej() async {
    ...
    try {
      final docNast = await FirebaseFirestore.instance.collection('nastaveni_servisu').doc(globalServisId).get();
      ...
      // Inkrementální generování sekvenčního čísla
      ...
      for (var p in _kosik) {
        // Pokud je položka vedená ve skladu (není to ruční zadání z volné ruky), odečteme ji
        if (p['sklad_id'] != null) {
          await FirebaseFirestore.instance.collection('sklad').doc(p['sklad_id']).update({
        ...
        polozkyProFakturu.add({
          ...
          'sleva': p['sleva'] ?? 0.0,
          'sklad_id': p['sklad_id'], // Ukládáme i sklad_id pro případná pozdější storna
        });
      }
      ...
      Reference pdfRef = FirebaseStorage.instance.ref().child('servisy/$globalServisId/faktury/$cisloFaktury.pdf'); 
      await pdfRef.putData(pdfBytes, SettableMetadata(contentType: 'application/pdf'));
      String pdfUrl = await pdfRef.getDownloadURL();

      await FirebaseFirestore.instance.collection('faktury').doc('${globalServisId}_$cisloFaktury').set({
        'servis_id': globalServisId,
        ...
        'datum_splatnosti': Timestamp.fromDate(ted.add(Duration(days: _formaUhrady == 'Převodem' ? 14 : 0))),
        'forma_uhrady': _formaUhrady,
        'celkova_castka': _celkemKosik,
        'stav_platby': (_formaUhrady == 'Hotově' || _formaUhrady == 'Kartou') ? 'Uhrazeno' : 'Čeká na platbu',
        'pdf_url': pdfUrl,
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Prodej byl úspěšně dokončen, faktura vytvořena.'), backgroundColor: Colors.green));
      }
    } catch (e) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Chyba prodeje: $e'), backgroundColor: Colors.red));
    } finally {
      setState(() => _isSaving = false);
    }
  }
                      ...
                      children: [
                        const Row(
                          children: [
                            ...
                            SizedBox(width: 10),
                            Text('Zákazník (nepovinné)', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                          ],
                        ),
                        const SizedBox(height: 15),
                        StreamBuilder<QuerySnapshot>(
                          ...
                          builder: (context, snapshot) {
                            ...
                            return DropdownButtonFormField<String>(
                              ...
                              items: zakaznici.map<DropdownMenuItem<String>>((z) {
                                ...
                                return DropdownMenuItem<String>(value: d['id_zakaznika']?.toString(), child: Text(d['jmeno']?.toString() ?? '---'));
                              }).toList(),
                              onChanged: (val) {
                                if (val != null) {
                                  final selected = zakaznici.firstWhere((z) => (z.data() as Map)['id_zakaznika']?.toString() == val);
                                  final data = selected.data() as Map<String, dynamic>;
                                  setState(() {
                                    _vybranyZakaznik = data;
                                    _jmenoCtrl.text = data['jmeno']?.toString() ?? '';
                                    _telefonCtrl.text = data['telefon']?.toString() ?? '';
                                    _emailCtrl.text = data['email']?.toString() ?? '';
                                    _adresaCtrl.text = data['adresa']?.toString() ?? '';
                                    _icoCtrl.text = data['ico']?.toString() ?? '';
                                    _dicCtrl.text = data['dic']?.toString() ?? '';
                                  });
                                }
                              },
                            );
                        ...
                        const SizedBox(height: 15),
                        _buildInput(controller: _jmenoCtrl, label: 'Jméno a Příjmení / Název firmy', icon: Icons.badge),
                        const SizedBox(height: 10),
                        Row(
                          children: [
                            Expanded(child: _buildInput(controller: _telefonCtrl, label: 'Telefon', icon: Icons.phone)),
                            const SizedBox(width: 10),
                            Expanded(child: _buildInput(controller: _emailCtrl, label: 'E-mail', icon: Icons.email)),
                          ],
                        ),
                        const SizedBox(height: 10),
                        _buildInput(controller: _adresaCtrl, label: 'Adresa', icon: Icons.location_on),
                        const SizedBox(height: 10),
                        Row(
                          children: [
                            Expanded(child: _buildInput(controller: _icoCtrl, label: 'IČO', icon: Icons.business)),
                            const SizedBox(width: 10),
                            Expanded(child: _buildInput(controller: _dicCtrl, label: 'DIČ', icon: Icons.account_balance_wallet)),
                          ],
                        ),
                      ],
                    ),
                      ...
                      children: [
                        const Row(
                          children: [
                            ...
                            SizedBox(width: 10),
                            Text('Výběr dílů z regálu', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                          ],
                        ),
                        const SizedBox(height: 15),
                        StreamBuilder<QuerySnapshot>(
                                ...
                                Row(
                                  ...
                                  children: [
                                    Column(
                                      crossAxisAlignment: CrossAxisAlignment.start,
                                      children: [
                                        Text('Název: ${_vybranyDilData!['nazev']}', style: const TextStyle(fontWeight: FontWeight.bold)),
                                        Text('Kód/OEM: ${_vybranyDilData!['kod'] ?? '-'}', style: const TextStyle(fontSize: 12, color: Colors.grey)),
                                      ],
                                    ),
                                    Column(
                                      crossAxisAlignment: CrossAxisAlignment.end,
                                      children: [
                                        Text('Dostupné: ${_vybranyDilData!['skladem']} ${_vybranyDilData!['jednotka']}', style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.orange)),
                                        Text('Cena: ${_vybranyDilData!['cena_prodej'] ?? 0} Kč', style: const TextStyle(fontSize: 14, color: Colors.green, fontWeight: FontWeight.bold)),
                                      ],
                                    ),
                                  ],
                                ),
                        ...
                        Row(
                          children: [
                            Expanded(
                              flex: 3,
                              child: _buildInput(controller: _mnozstviCtrl, label: 'Množství', icon: Icons.format_list_numbered, isNumber: true),
                            ),
                            const SizedBox(width: 10),
                            Expanded(
                              flex: 3,
                              child: _buildInput(controller: _slevaCtrl, label: 'Sleva %', icon: Icons.local_offer, isNumber: true),
                            ),
                            const SizedBox(width: 15),
                            Expanded(
                              flex: 4,
                              child: SizedBox(
                                ...
                                child: ElevatedButton.icon(
                                  ...
                                  label: const Text('DO KOŠÍKU', style: TextStyle(fontWeight: FontWeight.bold)),
              ...
              children: [
                Expanded(
                  child: Column(
                    ...
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      Text('V košíku: ${_kosik.length} položek', style: const TextStyle(color: Colors.grey, fontSize: 12, fontWeight: FontWeight.bold)),
                      Text('${_celkemKosik.toStringAsFixed(2)} Kč', style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: Colors.green)),
                    ],
                  ),
                ),
                ElevatedButton.icon(
                  ...
                  label: const Text('ZOBRAZIT A DOKONČIT', style: TextStyle(fontWeight: FontWeight.bold)),
                  style: ElevatedButton.styleFrom(
...
class _HistorieSkladuTab extends StatelessWidget {
  ...
  Future<DocumentSnapshot?> _fetchRelatedDoc(String zakazkaId) async {
    ...
    var doc = await FirebaseFirestore.instance.collection('zakazky').doc('${globalServisId}_$zakazkaId').get();
    if (doc.exists) return doc;
    
    doc = await FirebaseFirestore.instance.collection('faktury').doc('${globalServisId}_$zakazkaId').get();
    if (doc.exists) return doc;
            ...
            children: [
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(isPrijem ? 'Příjemka na sklad' : 'Výdejka ze skladu', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                        Text(_formatDate(data['datum']), style: const TextStyle(color: Colors.grey)),
                      ],
                    ),
                  ),
                  Text(
                    '$znaminko${data['mnozstvi']}', 
                    style: TextStyle(fontWeight: FontWeight.bold, fontSize: 26, color: color)
                  ),
                ],
              ),
              const SizedBox(height: 30),
              ...
              Card(
                ...
                child: Padding(
                  ...
                  child: Column(
                    children: [
                      _buildDetailRow('Název dílu:', data['nazev_dilu'] ?? '-'),
                      const Divider(height: 20),
                      _buildDetailRow(isPrijem ? 'Dodavatel / Pozn.:' : 'Poznámka:', data['poznamka'] ?? '-'),
                      if (isPrijem && data['cena_nakup'] != null) ...[
                        const Divider(height: 20),
                        _buildDetailRow('Nákupní cena:', '${data['cena_nakup']} Kč / ks'),
                      ],
                    ],
                  ),
                ),
              ...
              if (zakazkaId.isNotEmpty) ...[
                ...
                const Text('Vazba na doklad', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                const SizedBox(height: 10),
                ...
                FutureBuilder<DocumentSnapshot?>(
                  ...
                  builder: (context, snap) {
                    ...
                    final isFaktura = snap.data!.reference.path.contains('faktury');
                    
                    String zakName = '-';
                    if (isFaktura) {
                      zakName = rData['zakaznik_jmeno'] ?? '-';
                    } else {
                      zakName = (rData['zakaznik'] as Map<String, dynamic>?)?['jmeno'] ?? '-';
                    }
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        leading: CircleAvatar(
                          ...
                          foregroundColor: Colors.white,
                          child: Icon(isFaktura ? Icons.receipt_long : Icons.build),
                        ),
                        title: Text(isFaktura ? 'Faktura: ${rData['cislo_faktury']}' : 'Zakázka: ${rData['cislo_zakazky']}', style: const TextStyle(fontWeight: FontWeight.bold)),
                        subtitle: Text('Zákazník: $zakName\n${rData['spz'] != null && rData['spz'].toString().isNotEmpty ? 'Vozidlo: ${rData['spz']}' : ''}'),
                        isThreeLine: rData['spz'] != null && rData['spz'].toString().isNotEmpty,
                          ...
                          if (isFaktura) {
                            Navigator.push(context, MaterialPageRoute(
                              builder: (context) => FakturaDetailScreen(
                                ...
                                zakazkaId: rData['cislo_zakazky']?.toString() ?? '',
                              )
                            ));
                          } else {
                            Navigator.push(context, MaterialPageRoute(
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        if (docs.isEmpty) {
          return const Center(child: Text('Zatím neevidujeme žádné skladové pohyby.', style: TextStyle(color: Colors.grey)));
        }
            ...
            return InkWell(
              ...
              child: Padding(
                ...
                child: ListTile(
                  ...
                  subtitle: Text('${_formatDate(data['datum'])} | ${data['poznamka'] ?? (data['zakazka_id'] != null ? 'Zakázka ${data['zakazka_id']}' : '-')}'),
                  trailing: Row(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      Text(
